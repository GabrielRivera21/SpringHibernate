package com.deeplogics.cloud.client;
			...
			if (!loggedIn) {
				try {
                    ...
                    if(refreshToken == null) {
                        ...
                        //encode the Refresh token into the body of the request
					...
					// by adding grant_type=password to the body or OAuth Refresh Token Flow
                    // by adding grant_type=refresh_token
					...
					if (resp.getStatus() < 200 || resp.getStatus() > 299) {
						...
						// Extract the access_token (bearer token) and refresh_token
						// from the response so that we can add it to future requests.
